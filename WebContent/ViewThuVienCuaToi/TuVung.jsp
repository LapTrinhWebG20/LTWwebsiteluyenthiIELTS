         ...
         <form  method="get" action="/FileRead">
		   <table border="0">
			  	<tr>
				    ...
				    <th id="delete" style="color:lightseagreen">File Type</th>
				    <th id="delete" style="color:lightseagreen">Action</th>
				    <th style="color:lightseagreen">View</th>
				</tr>
			</br>
			<legend style="color:blue">TỪ VỰNG IELTS</legend>
<%
	  //Lấy tên người dùng hiện tại
      String ma_nd=(String)request.getSession().getAttribute("_user");
	  //Truy vấn lấy tài liệu của người dùng
	  String query = "select doc_id, filename,tenfile from document where madulieu=11111 and MaNguoiDung='"+ma_nd+"'";
	  ResultSet rs = DataProvider.getResultFromSqlQuery(query);
	  
	  int count =0;//Biến đếm tài liệu
	  while(rs.next())
	  {
	    out.println("<tr>"
	        + "<td>"+rs.getString(2)+"</td>"//Lấy tên bài đăng
	        + "<td>"+rs.getString(3)+"</td>"//Lấy tên file
	        + "<td>"
	        +   "<a href='ViewThuVienCuaToi/deletefile.jsp?doc_id="+rs.getInt(1) +"'> Delete </a>"//Xóa file có doc_id lấy ở trên
	        + "</td>"
	        +"<td>"
	        + "<a target='_blank' href='FileRead?doc_id="+rs.getInt(1)+"'>View</a>"//View file có doc_id đươc lấy ở trên
	        + "</td>"
	        + "</tr>");
	    count++;
	  }
