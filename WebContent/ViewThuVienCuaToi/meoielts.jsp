...
%>
    <div id="q">
         <form  method="get" action="/FileRead">
		  ...
		  <legend style="color:blue">MẸO THI IELTS</legend>
		<%
			//Lấy tên người dùng hiện tại
		    String ma_nd=(String)request.getSession().getAttribute("_user");
		    
			//Truy vấn lấy dữ liệu các file thuộc Mẹo Thi
			String query = "select doc_id, filename,tenfile from document where madulieu=11113 and MaNguoiDung='"+ma_nd+"'";
			ResultSet rs = DataProvider.getResultFromSqlQuery(query);//Thực hiện truy vấn
			
		  	int count =0;
		  	while(rs.next())
		  	{
		    	out.println("<tr>"
		        	+ "<td>"+rs.getString(2)+"</td>"//In file name
		        	+ "<td>"+rs.getString(3)+"</td>"//In tên file
			        + "<td>"
			        +   "<a href='ViewThuVienCuaToi/deletemeothi.jsp?doc_id="+rs.getInt(1) +"'> Delete </a>"//Xóa file với giá trị doc_id
			        + "</td>"
			        +"<td>"
			        + "<a target='_blank' href='FileRead?doc_id="+rs.getInt(1)+"'>View</a>"//View file với giá trị doc_id
			        + "</td>"
			        + "</tr>");
		    	count++;
		  	}
